...
def test_convert_usd_to_rub(mock_get):
    ...
    mock_get.return_value.json.return_value = {"rates": {"RUB": 100.0}}  # курс 1 USD = 100 рублей
...
def test_unsupported_currency():
    transaction = {"operationAmount": {"amount": "200.00", "currency": {"code": "GBP"}}}
...
def test_api_failure(mock_get):
    ...
    transaction = {"operationAmount": {"amount": "20.00", "currency": {"code": "USD"}}}
...
def test_missing_operation_amount():
    transaction = {"description": "Платёж без суммы"}
...
def test_invalid_amount_value():
    transaction = {"operationAmount": {"amount": "abc", "currency": {"code": "RUB"}}}  # некорректное значение
...
def test_operation_amount_is_none():
    transaction = {"operationAmount": None}
