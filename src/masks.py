...
def get_mask_card_number(card_number: str) -> str:
    ...
    if not isinstance(card_number, str):
        logger.error("Ошибка номер карты должен быть строкой")
        raise TypeError("Номер карты должен быть строкой")

    if card_number == "":
        logger.error("Ошибка номер карты не может быть пустым")
        raise ValueError("Номер карты не может быть пустым")

    if len(card_number) != 16:
        logger.error("Ошибка номер карты должен содержать ровно 16 цифр")
        raise ValueError("Номер карты должен содержать ровно 16 цифр")

    if not card_number.isdigit():
        logger.error("Ошибка номер карты должен содержать только цифры")
...
def get_mask_account(account_number: str) -> str:
    ...
    if not isinstance(account_number, str):
        logger.error("Ошибка номер счёта счёта должен быть строкой")
        raise TypeError("Номер счёта должен быть строкой")

    if account_number == "":
        logger.error("Ошибка номер счёта не может быть пустым")
        raise ValueError("Номер счёта не может быть пустым")

    if len(account_number) != 20:
        logger.error("Ошибка номер счёта должен содержать ровно 20 цифр")
